...
def filter_data(probe1, probe2, time, goda_start, goda_end, step):
    """
    过滤数据，只保留指定时间范围内的数据
    """
    ...
    return time_segment, probe1_segment, probe2_segment
...
def neo_fit(x, y, Tw):
    ...
    进行傅里叶拟合
    """
    def fourier_model(t, a0, *coeffs):
        w = coeffs[-1]  # 从参数列表中提取 w
        y_vals = a0
        for k in range(1, 9):
            a = coeffs[k - 1]
            b = coeffs[k + 7]
    ...
    initial_guess = [np.mean(y)] + [0.1] * 16 + [2 * np.pi / Tw]
...
def goda_test(wave_period, water_depth, probe1, probe2, time, deltaL, goda_start, goda_end, step):
    ...
    主测试函数
    """
    time_segment, probe1_segment, probe2_segment = filter_data(probe1, probe2, time, goda_start, goda_end, step)
