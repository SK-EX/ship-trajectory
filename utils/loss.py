...
class MultiBoxLoss:
    def __init__(self,num_classes, alpha = 1.0,neg_pos_ratio = 3.0,background_label_id=0, negatives_for_hard=100.0 ):
        ...
        if background_label_id != 0:
            print("background_label_id must be 0!")
        else:
    ...
    def _l1_smooth_loss(self, y_true, y_pred):
        ...
        #假设我们有一个形状为 (batch_size, num_classes) 的张量 loss ，
        #代表了一个批次中每个样本在各个类别上的损失值
        #（比如在分类任务中，每个样本对应不同类别有不同的交叉熵损失等情况）。
        abs_loss  =  torch.abs(y_pred - y_pred)
        sq_loss = 0.5 * (y_pred - y_true) ** 2
        loss = torch.where(abs_loss < 1 , sq_loss, abs_loss - 0.5)
